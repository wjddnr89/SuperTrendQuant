...
def run_hell_backtest_v2():
    print("==========================================================")
    print("🔥 [조율 A 반영] 2022 대폭락장 관통 복리 백테스트 (Ver 2.0)")
    print("🎯 수정 사항: 일봉 기준 ATR 익절 타겟 배수 하향 (6.0 -> 2.5)")
    print("==========================================================")
    ...
    # [조율 A 핵심 파라미터]
    ATR_TARGET_MULTIPLIER = 2.5  # 기존 6.0에서 2.5로 대폭 현실화
    ...
    for date in all_dates:
        ...
        for t in list(positions.keys()):
            ...
            # 🎯 조율 A: 수정된 가변 익절라인 검사 (진입가 + ATR * 2.5)
            ...
            # 본전 청산 혹은 SuperTrend 매도 전환 시 청산
        ...
        for ticker in universe:
            ...
            if df['Trend'].iloc[idx] == 1 and df['Trend'].iloc[idx-1] == 1 and (df['Volume'].iloc[idx] > df['Vol_MA20'].iloc[idx-1]) and (df['ADX'].iloc[idx] >= 20):
