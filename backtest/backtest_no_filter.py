...
# ==========================================
# ⚙️ 유저님 오리지널 환경 세팅 (필터만 제외)
# ==========================================
# 하락장에도 굳이 SOXS로 도망치지 않고, 14개 전 종목을 상시 매수 후보군으로 개방
...
print("⏳ 야후 파이낸스로부터 30분봉 데이터 수집 중 (마켓 필터 제외 버전)...")
raw_data = yf.download(FULL_UNIVERSE + ["QQQ"], period="1mo", interval="30m", progress=False)
...
for ticker in FULL_UNIVERSE + ["QQQ"]:
    ...
    df['Return_5d'] = df['Close'].pct_change(65) # 30분봉 65개 = 약 5영업일
    data_dict[ticker] = df

# ==========================================
# 💰 시뮬레이터 구동 (QQQ EMA200 무시)
# ==========================================
cash = START_CASH
positions = {}
trade_history = []
...
for idx in all_indices:
    ...
    # [후보군 빌딩] 대세 필터 없이 14개 종목 전체를 상시 모니터링
    all_candidates = []
...
annual_factor = 13 * 252
if port_df['returns'].std() != 0:
    sharpe_ratio = (port_df['returns'].mean() / port_df['returns'].std()) * np.sqrt(annual_factor)
else:
    sharpe_ratio = 0.0

port_df['peak'] = port_df['total_value'].cummax()
port_df['drawdown'] = (port_df['total_value'] - port_df['peak']) / port_df['peak']
mdd = port_df['drawdown'].min() * 100

if trade_history:
    th_df = pd.DataFrame(trade_history)
    win_rate = (th_df['success'].sum() / len(th_df)) * 100
    
    avg_win = th_df[th_df['pnl_pct'] > 0]['pnl_pct'].mean() if th_df['pnl_pct'].max() > 0 else 0
    avg_loss = abs(th_df[th_df['pnl_pct'] < 0]['pnl_pct'].mean()) if th_df['pnl_pct'].min() < 0 else 1
...
print("\n" + "="*45)
print(f" 📈 마켓 필터 제외 버전 백테스트 결과 보고서")
print("="*45)
print(f"🔥 1. 누적 수익률   : {total_return:.2f}%")
print(f"🔥 2. 샤프 비율     : {sharpe_ratio:.2f}")
print(f"🔥 3. 승률          : {win_rate:.2f}% (총 {len(trade_history)}회 청산 완료)")
