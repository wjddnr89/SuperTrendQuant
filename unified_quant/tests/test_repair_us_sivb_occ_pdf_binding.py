...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "repair_us_sivb_occ_pdf_binding.py"
)
SPEC = importlib.util.spec_from_file_location(
    "repair_us_sivb_occ_pdf_binding", SCRIPT_PATH
)
...
PAGE_ONE = [
    ...
    "New Symbol: SIVBQ",
    "Date: 03/28/2023",
    "SVB Financial Group (SIVB) will change its trading symbol to SIVBQ",
    "effective March 28, 2023, due to the listing of the company on an OTC market.",
    "As a result, option symbol SIVB will change to SIVBQ effective at the",
    "opening of business on March 28, 2023.",
    "Strike prices and all other option terms will not change.",
    "Clearing Member input to OCC must use the new option symbol SIVBQ",
    "commencing March 28, 2023.",
    "Date: March 28, 2023",
    "Option Symbol: SIVB changes to SIVBQ",
    "Underlying Security: SIVB changes to SIVBQ",
                ...
                "metadata": json.dumps(
                    {
                        ...
                        "nasdaq_suspension_date": script.EFFECTIVE_DATE,
                        "occ_reviewed_extraction_hash": (
                            script.LEGACY_REVIEWED_EXTRACTION_SHA256
                        ),
                        "otc_open_date": script.EFFECTIVE_DATE,
                        "policy": (
                            "legal_cancellation_and_market_transition_separation/v1"
                        ),
                        "ratio": 1.0,
                        "same_common_share_identity": True,
                        "sec_market_source_hash": "a" * 64,
                    },
                    sort_keys=True,
                    separators=(",", ":"),
                ),
            ...
            [
                {
                    ...
                    "effective_to": "2023-03-27",
                    "source": "fixture",
                    "retrieved_at": RETRIEVED_AT,
                    "source_hash": hashlib.sha256(b"history-sivb").hexdigest(),
                },
                {
                    "security_id": script.SECURITY_ID,
                    ...
                    "source": "fixture",
                    "retrieved_at": RETRIEVED_AT,
                    "source_hash": hashlib.sha256(b"history-sivbq").hexdigest(),
                },
            ]
        ),
        completed_session=COMPLETED_SESSION,
...
def test_pdf_requires_exact_claims_and_visual_origin_attestation(tmp_path: Path):
    path = _write_pdf(tmp_path / "occ-52179.pdf")
...
def test_pdf_rejects_missing_claim_and_wrong_page_count(tmp_path: Path):
    missing = [line for line in PAGE_ONE if "78486Q101" not in line]
    with pytest.raises(script.EvidenceError, match="cusip_78486q101"):
