...
def run_v596_hourly_backtest():
    print("==========================================================")
    print("⏰ [Ver 5.9.6-Hourly] 1시간봉 추세 무력화 검증 엔진 가동")
    print("✅ 코어 사양: 극강의 가성비 2슬롯 체제 (MAX_SLOTS = 2) 유지")
    print("✅ 타임프레임 변환: 기존 30분봉 -> 60분봉(1시간봉)으로 무겁게 튜닝")
    print("==========================================================")
    ...
    # 💡 [타임프레임 변경] 30m -> 1h (60일간의 1시간 시계열 데이터 로드)
    raw_data = yf.download(full_universe + ["QQQ"], period="60d", interval="1h", progress=False)
    ...
    for ticker in full_universe:
        try:
            ...
            # 💡 1시간봉 기준 5영업일은 대략 33~34개 봉에 해당 (6.5시간 * 5일)
            df['Return_5d'] = df['Close'].pct_change(33)
    ...
    qqq_close_1h = raw_data['Close']['QQQ'].dropna()
    qqq_ret_5d = qqq_close_1h.pct_change(33)
    
    initial_cash = 10000.0
    cash = initial_cash
    positions = {} 
    MAX_SLOTS = 2  # 💡 2슬롯 극대화 체제 유지
    FEE_HALF = 0.00225
    SLIPPAGE = 0.0005
    HURDLE_RATE = 0.015
    ...
    for idx, date in enumerate(all_dates):
        ...
        if all_candidates:
            ...
            for t in list(positions.keys()):
                if t in top_2_tickers or t in [o['ticker'] for o in pending_orders]: continue
            ...
            for candidate in all_candidates:
                ...
                if t not in positions and candidate['signal_buy']:
                    pending_orders.append({'ticker': t, 'type': 'BUY'})
    ...
    print("\n==========================================================")
    print("      🎯 [최종 성적표] Ver 5.9.6-Hourly 1시간봉 결과")
    print("==========================================================")
