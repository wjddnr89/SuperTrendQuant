...
def load_split_config(
    ...
    strategy_raw = _load_mapping(Path(strategy_path))
    runtime_raw = _load_mapping(Path(runtime_path))
...
def compose_split_config(
    ...
    composed = {
        ...
        "exit": {
            "sell_confirm_bars": _find_component_value(strategy_raw, "supertrend_flip", "confirm_bars", default=1),
        },
        "execution": {
...
def _compose_indicators(strategy_raw: dict[str, Any]) -> dict[str, Any]:
    supertrend = _find_component(strategy_raw, "supertrend") or {}
    return {
        "supertrend": {
...
def _validate_component_keys(group_name: str, component_type: str, raw: dict[str, Any]) -> None:
    allowed = {
        ...
        ("filters", "relative_strength"): {"type", "lookback_bars"},
        ("exits", "supertrend_flip"): {"type", "confirm_bars"},
