...
# 허들 레이트 상향 및 마켓별 RS_PERIOD 이원화
HURDLE_ATR_MULT = 1.5       
KR_RS_PERIOD = 100          # 국장: 약 7.5거래일 트렌드 추종 (더 기민하게 반응)
US_RS_PERIOD = 130          # 미장: 기존 10거래일 유지 (안정적인 대장주 포착)
    ...
    def cancel_order(self, order_id):
        ...
        try:
            ...
            if res.status_code in [200, 204]:
                print(f"🗑️ 구형 미체결 주문 취소 완료. ID: {order_id}")
                return True
            else:
    ...
    def send_order(self, symbol, side, qty, order_type="market", price=None):
        ...
        try:
            ...
            if res.status_code in [200, 201]:
                print(f"✅ 토스 공식 주문 전송 성공: {symbol} | {side} | {qty}주 | 타입: {order_type}")
                return True
            else:
...
class HybridTradingBot:
    ...
    async def execute_trade_cycle(self):
        ...
        for s in account_data.get("stocks", []):
            ...
            if sym:
                real_holding_stocks[sym] = {
                    ...
                    "buy_price": float(s.get("averagePurchasePrice", 0)) # 장부 가독성을 위해 buy_price 명칭 통일
                }
        ...
        for t in list(real_holding_stocks.keys()):
            if real_holding_stocks[t]['qty'] > 0 and t in current_universe_keys:
                market_holdings[t] = {
                    ...
                    "buy_price": real_holding_stocks[t]['buy_price']
                }
        ...
        if not is_close_briefing:
            ...
            for order in open_orders:
                ...
                if side == "buy" and sym in realtime_prices:
                    # 시장가 주문 체결 위주 환경이므로 구형 지정가 추적 유실 방지용 기존 로직 유지
        ...
        for b in benchmarks:
            ...
            if len(b_close) >= current_rs_period:
                bench_ret_30m[b] = b_close.pct_change(current_rs_period)
        ...
        for symbol, mkt_type in scan_source:
            ...
            # 🛠️ [지표 착시 교정] 종가 덮어쓰기를 전면 폐기하고 가상의 새로운 행을 하단에 append
            if symbol in realtime_prices:
                toss_price = realtime_prices[symbol]
                pseudo_time = df_orig.index[-1] + pd.Timedelta(seconds=1)
                df_orig.loc[pseudo_time] = [toss_price, toss_price, toss_price, toss_price]
            ...
            if bench_symbol in bench_ret_30m:
                df['RS'] = df['Close'].pct_change(current_rs_period) - bench_ret_30m[bench_symbol]
            else: continue
        ...
        # [매도 룰 1단계: Supertrend 추세 이탈 - 국장/미장 시장가 전면 최적화]
        for t in list(bot_holding_stocks.keys()):
            ...
            if processed_data[t]['Trend'].iloc[-1] == -1:
                ...
                # 🌟 국장/미장 모두 칼 같은 청산을 위해 전면 시장가 주문 실행
                success = self.account_mgr.send_order(symbol=t, side="sell", qty=order_qty, order_type="market")
                if success:
                    send_telegram(f"🚨 *[추세 이탈 매도]*\n• 종목: {t} | {order_qty}주 전량 청산 (추세 하락 전환)")
                    del market_holdings[t]
                    holdings_master[target_market] = market_holdings
                    save_holdings(holdings_master)
                    has_action = True

        # [매도 룰 2단계: 순환매 주도주 교체 - 비교 착시 대폭 교정]
        if all_candidates and bot_holding_stocks:
            for t in list(bot_holding_stocks.keys()):
                if t not in processed_data or t in existing_order_symbols: continue
                
                # 🛠️ [비교 착시 교정 1] 후보군 리스트 누락 시에도 연산 메모리 내 실시간 RS 정상 추적
                current_rs = next((c['rs'] for c in all_candidates if c['ticker'] == t), None)
                if current_rs is None and 'RS' in processed_data[t].columns:
                    current_rs = processed_data[t]['RS'].iloc[-1]
                ...
                # 🛠️ [비교 착시 교정 2] 점수 멜트다운(-999.0) 혹은 수익률 +5% 안전마진 미만 시 잦은 뇌동 교체 강력 억제
                if current_rs == -999.0 or profit_pct < 0.05:
                    # 단, 점수가 정상이고 새 주도주가 기존 허들을 압도적으로 상회할 시에만 예외 허용
                    if current_rs != -999.0 and (best_new['rs'] - current_rs > (best_new['atr_pct'] * HURDLE_ATR_MULT)):
                        pass
                    else:
                        continue # 안전 브레이크 작동: 교체하지 않고 홀딩

                # 교체 허들 최종 통과 시 시장가 청산
                old_qty = bot_holding_stocks[t]['qty']
                
                # 🌟 국장/미장 신속 교체를 위해 전면 시장가 주문 실행
                sell_success = self.account_mgr.send_order(symbol=t, side="sell", qty=old_qty, order_type="market")
                if sell_success:
                    send_telegram(f"🔄 *[순환매 교체]*\n• 주도주 교체: {t} 매도 완료 ➡️ {best_new['ticker']} 진입 대기\n• 직전 수익률: {profit_pct*100:.2f}%")
                    del market_holdings[t]
                    holdings_master[target_market] = market_holdings
                    save_holdings(holdings_master)
                    has_action = True

        # [매수 룰: 압도적 대장주 슬롯 집중 진입 - 국장 지정가 제거 및 시장가 일원화]
    ...
    async def start_trading_bot(self):
        send_telegram("🤖 *Ver 24.2 실시간 데이터 착시 보정 및 전면 시장가 패치 완료!*")
        while True:
            try: 
