...
def run_v595_backtest():
    print("==========================================================")
    print("🛡️ [Ver 5.9.5] '갭 가드' 시가 매수 방어형 퀀트 엔진 가동")
    print("✅ 코어 사양: 3종목 집중 투자 체제 (MAX_SLOTS = 3) 유지")
    print("✅ 위험 관리: 장 시작 첫 30분(09:30) 무지성 갭 상승 추격매수 원천 금지")
    print("✅ 완화 룰 적용: 트레일링 스탑 (+30% / -10%), 순환매 허들 (+1.5%)")
    print("==========================================================")
    ...
    for idx, date in enumerate(all_dates):
        ...
        # 💡 장 시작 첫 봉(미국 시간 기준 09:30분봉) 여부 감지 판별기
        is_opening_bell = (date.strftime('%H:%M') == '09:30')
        
        current_qqq_ema200 = qqq_ema_map.loc[date_str] if date_str in qqq_ema_map.index else qqq_ema_map.iloc[-1]

        # [1. 매도 주문 체결] (매도는 장 시작 시초가에도 탈출해야 하므로 필터 제외)
        ...
        for order in [o for o in pending_orders if o['type'] == 'BUY']:
            ...
            if t not in positions and len(positions) < MAX_SLOTS:
                ...
                # 💡 [핵심 이식] 매수 주문이 쌓여있더라도 장 시작 첫 봉(09:30)이면 집행을 유예하고 Skip합니다!
                if is_opening_bell:
                    continue
        ...
        if all_candidates:
            ...
            for candidate in all_candidates:
                ...
                if t not in positions and candidate['signal_buy']:
                    # 첫 30분봉 시가 매수가 아닐 때만 큐에 진입 주문 수락
                    if not is_opening_bell:
    ...
    print("\n==========================================================")
    print("      🎯 [최종 성적표] Ver 5.9.5 '갭 가드' 엔진 결과")
    print("==========================================================")
