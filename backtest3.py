...
def run_hell_backtest_v3():
    print("==========================================================")
    print("🔥 [조율 B+C 반영] 2022 대폭락장 관통 복리 백테스트 (Ver 3.0)")
    print("🎯 수정 1: 가짜 추세 필터링을 위해 진입 ADX 장벽 상향 (20 -> 30)")
    print("🎯 수정 2: 흔들기 방지를 위해 본전 청산 유예 버퍼 도입 (-1.5%)")
    print("==========================================================")
    ...
    ADX_ENTRY_BARRIER = 30       # 조율 C: ADX 문턱을 30으로 대폭 상향
    TRAILING_BUFFER = 0.015       # 조율 B: 진입가 대비 -1.5%까지는 숨쉴 틈을 줌
    ...
    for date in all_dates:
        ...
        for t in list(positions.keys()):
            ...
            # 🚨 조율 B 반영: 무조건 본전이 아니라 진입가의 -1.5% 버퍼선 붕괴 시 청산
            trailing_stop_line = entry_p * (1 - TRAILING_BUFFER)
        ...
        if buy_candidates:
            ...
            if (is_lev and lev_slots < 8) or (not is_lev and norm_slots < 4):
                if qty > 0 and cash >= cost:
                    cash -= cost
                    positions[t_ticker] = {'qty': qty, 'entry_price': best['price'], 'atr': best['atr'], 'half_exit': False, 'is_leverage': is_lev, 'adx': best['adx']}
            else:
    ...
    print("\n==========================================================")
    print("      🎯 [Ver 3.0] 조율 B+C 결합 최종 스코어보드")
    print("==========================================================")
