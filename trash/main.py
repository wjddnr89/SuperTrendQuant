...
class HybridTradingBot:
    ...
    def sync_yahoo_finance_state(self, yf_tickers, benchmarks):
        ...
        try:
            ...
            for yf_tk in yf_tickers:
                if yf_tk in raw_30m['Close']: cached_data_30m[yf_tk] = pd.DataFrame({'Open': raw_30m['Open'][yf_tk], 'High': raw_30m['High'][yf_tk], 'Low': raw_30m['Low'][yf_tk], 'Close': raw_30m['Close'][yf_tk]}).dropna().copy()
    ...
    async def execute_trade_cycle(self):
        ...
        if last_candle_base_time is None or last_candle_base_time != current_candle_base:
            if self.sync_yahoo_finance_state(yf_tickers, benchmarks): last_candle_base_time = current_candle_base
        ...
        for b in benchmarks:
            ...
            if b in cached_bench_1h and not cached_bench_1h[b].empty: upper_trends_1h[b] = calculate_supertrend(cached_bench_1h[b].copy(), period=10, multiplier=3.0)
