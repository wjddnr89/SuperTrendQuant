...
# =====================================================================
# 📦 [Stateful 메모리 관리 레이어 - 추가]
# =====================================================================
# 야후파이낸스로부터 받아온 과거 30일치 원본 시계열 데이터를 봇이 켜져 있는 동안 RAM에 유지합니다.
...
class HybridTradingBot:
        ...
        try:
            ...
            for yf_tk in yf_tickers:
                if yf_tk in raw_30m['Close']:
                    ...
                    # 종목 코드 분리 매핑을 위해 원래 키값 형태로 복사본 캐싱
    ...
    async def execute_trade_cycle(self):
        ...
        # 첫 구동이거나, 30분 타임블록이 새로 바뀌었을 때만 야후 다운로드 트리거
        ...
        # -----------------------------------------------------------------
        # 🌟 [네트워크 차단 완화] 토스 Open API 다건 현재가 일괄 동기화 (Batch)
        # -----------------------------------------------------------------
        realtime_prices = {}
        ...
        for b in benchmarks:
            ...
            df_b = cached_bench_1h[b].copy()  # 원본 훼손 방지를 위해 카피본 사용
            if not df_b.empty:
        ...
        for symbol, mkt_type in scan_source:
            ...
            # 원본 데이터 캐시 프레임 보존을 위한 1분 소멸성 깊은 복사(Copy)
            ...
            if symbol in realtime_prices:
                ...
                # 야후 파이낸스 가장 최근 봉(-1 행)의 종가를 토스 실시간가로 강제 교체
                df.iloc[-1, df.columns.get_loc('Close')] = toss_price
                # 실시간가가 기존 고가/저가를 탈피할 경우 범위 확장 보정 (ATR 공식 왜곡 파괴 방지)
    ...
    async def start_trading_bot(self):
        send_telegram("🤖 *Ver 22.0 Stateful 고도화 및 하이브리드 스티칭 엔진 기동 완료!*")
        while True:
            try: 
