...
def test_actual_candidate_keeps_krft_yhoo_event_cross_validation_passed() -> None:
    ...
    if release is None or not set(script.REQUIRED_DATASETS).issubset(
        ...
        pytest.skip("Actual release lacks KRFT/YHOO transition datasets.")
    before = _control_bytes(repository)
    prepared = script.prepare_repair(repository)
    assert prepared.summary["status"] == "validated_offline_plan"
    assert _control_bytes(repository) == before
    ...
    report_hash = str(lifecycle_manifest.metadata["evidence_report_sha256"])
    report_path = REPOSITORY_ROOT / f"archives/{release.completed_session}/{report_hash}.json.gz"
    lifecycle_report = json.loads(gzip.decompress(report_path.read_bytes()))
    policy = cross_script.load_policy(
        Path(__file__).resolve().parents[1] / "configs/us_cross_validation.yaml"
    )
    current_checks = cross_script.build_event_checks(
    ...
    checks = cross_script.build_event_checks(
        prepared.frames["corporate_actions"],
        prepared.frames["lifecycle_resolutions"],
        lifecycle_report,
        prepared.frames["source_archive"],
        policy,
        lifecycle_report_sha256=report_hash,
    )
    current_by_id = {item["event_id"]: item for item in current_checks}
    by_id = {item["event_id"]: item for item in checks}
    for old_event_id, event_id in (
        (script.KRFT_OLD_EVENT_ID, script.KRFT_NEW_EVENT_ID),
        (script.YHOO_OLD_EVENT_ID, script.YHOO_NEW_EVENT_ID),
    ):
        assert current_by_id[old_event_id]["status"] == "passed"
        assert by_id[event_id]["status"] == "passed"
        assert by_id[event_id][
            "reviewed_terminal_market_date_correction_applied"
        ] is True
        assert by_id[event_id]["date_match"] is True
    assert sum(item["status"] != "passed" for item in checks) == sum(
        item["status"] != "passed" for item in current_checks
    )
    unaffected_current = {
        key: value["status"]
        for key, value in current_by_id.items()
        if key not in {script.KRFT_OLD_EVENT_ID, script.YHOO_OLD_EVENT_ID}
    }
    unaffected_candidate = {
        key: value["status"]
        for key, value in by_id.items()
        if key not in {script.KRFT_NEW_EVENT_ID, script.YHOO_NEW_EVENT_ID}
    }
    assert unaffected_candidate == unaffected_current
    assert _control_bytes(repository) == before
