...
class PaperConfig:
    ...
    quote_source: str = "completed_bar"
...
def compose_split_config(
    ...
    composed = {
        ...
        "paper": {
            ...
            "quote_source": paper_raw.get("quote_source", "completed_bar"),
            "telegram_enabled": paper_raw.get("telegram_enabled", False),
        },
        "backtest": {
...
def _parse_paper_config(raw: dict[str, Any]) -> PaperConfig:
    config = PaperConfig(
        **_known(
    ...
    if config.quote_source not in {"completed_bar", "toss"}:
        raise ValueError("paper.quote_source must be completed_bar or toss.")
    return config
